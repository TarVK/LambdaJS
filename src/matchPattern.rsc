...
data MatchTreeErrors = MTE(MatchTree tree, Errors errors);
public MatchTreeErrors createMatchTree(
	list[Const] constructors, 
...
public MatchTreeErrors createMatchTree(
	...
	if (
		...
		if(MTE(result, newErrors) := createMatchTree(constructors, newMatches, posStart + (prevIndex+1), constPathStart))
			return MTE(result, newErrors + errors);
	}
	
	// Finish if the first match is an end
	if (depth == 0) {
		if([<End(_, exp), Substitution sub, _>, *r] := matches)
			return MTE(ST(exp, sub), errors);
		if (size(matches) == 0) 
			return MTE(Undefined(), errors); 
	}
	...
	map[Const, MatchTree] patternMatchesMap = (
		constructor: {
			...
			MatchTree tree;
			if(MTE(result, newErrors) := createMatchTree(
				constructors,
				constMatches,
				pos + 0,
	...
	return MTE(returnTree, errors);
}



// Error handling types
alias Errors = set[Error];
